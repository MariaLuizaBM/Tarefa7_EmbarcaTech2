...
#include "hardware/pwm.h"           // Biblioteca de hardware para controle de PWM (modulação por largura de pulso)
...
#define WIFI_SSID "Wilma"                  // Substitua pelo nome da sua rede Wi-Fi
#define WIFI_PASSWORD "99896898"              // Substitua pela senha da sua rede Wi-Fi
#define MQTT_SERVER "192.168.0.7"                // Substitua pelo endereço do host - broket MQTT: Ex: 192.168.1.107
#define MQTT_USERNAME "malu"              // Substitua pelo nome da host MQTT - Username
#define MQTT_PASSWORD "malu"              // Substitua pelo Password da host MQTT - credencial de acesso - caso exista
...
void tocar_alarme() {
    for (int j = 0; j < 5; j++) {  // 5 beeps
        ...
        sleep_ms(200);  // Pausa entre os beeps
    }
}
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/led/11"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/led/12"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/led/13"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/alarme"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/festa"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strncmp(basic_topic, "/led/", 5) == 0) {
    int pin = atoi(&basic_topic[5]);
    int value = atoi(state->data);

    if (value < 0) value = 0;
    if (value > 255) value = 255;

    if (pin == LED_RED_PIN) {
        red_led_brightness = value;
    } else if (pin == LED_GREEN_PIN) {
        green_led_brightness = value;
    } else if (pin == LED_BLUE_PIN) {
        blue_led_brightness = value;
    } else {
        return; // Ignora pinos inválidos
    }

    atualizar_pwm_leds();  // Aplica os valores atualizados
    printf("Set LED PWM pin %d brightness %d\n", pin, value);
        ...
        tocar_alarme();
    } else if (strcmp(basic_topic, "/festa") == 0) {
        tocar_musica_festa();
    }
